...
const CreateChallenge = ({ closeModal }) => {
  ...
  const onNewChallengeClick = async () => {
    if (info && name && attendees.length > 0) {
    ...
    >
      <div>
        <div className="text-center mb-4 mt-5">
          ...
          <h1 className="sm:text-3xl text-2xl font-medium text-center title-font text-gray-500 mb-4">Create New Challenge</h1>
          <p className="text-base leading-relaxed xl:w-3/4 lg:w-3/4 mx-auto">Challenge Name:</p>
          <input
            // eslint-disable-next-line jsx-a11y/no-autofocus
            autoFocus
            className="m-5 p-1 py-2 px-2 border border-yellow-500 bg-gray-100"
            onChange={e => setName(e.target.value)}
            placeholder="Beautiful event"
            value={name}
          />
          <p className="text-base leading-relaxed xl:w-3/4 lg:w-3/4 mx-auto">Challenge Description:</p>
          <textarea
            className="m-5 p-1 py-2 px-2 border border-yellow-500 bg-gray-100"
            cols="40"
            onChange={e => setInfo(e.target.value)}
            placeholder="Beautiful event description"
            rows="3"
            value={info}
          />
          <br />
